...
contract MultiToken is ERC1155, Ownable {  
  ...
  function getType(uint256 _id) public pure returns(uint256) {
  ...
  function nextId(bool nonFungible) internal returns(uint256) {
    if (nonFungible) {
  ...
  function create(
    ...
    returns(uint256 _id) {
      ...
      emit Create(_id, nonFungible, initialSupply, data);
  }

  function nextNF(uint256 id) internal returns(uint256) {
